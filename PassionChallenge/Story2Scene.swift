...
class Story2Scene: SKScene{
    let story2Scene = SKScene(size: CGSize(width: 480, height: 320))
    let textureAtlas: SKTextureAtlas = SKTextureAtlas(named: "HUD1")
    let backgroundImageS2 = SKSpriteNode(imageNamed: "ceuNeon")
    let story2Text = SKLabelNode(fontNamed: "AvenirNext-Heavy")
    
    override func didMove(to view: SKView) {
        self.anchorPoint = CGPoint(x: 0.5, y: 0.5)
        
        backgroundImageS2.size = CGSize(width: 1024, height: 768)
